...
## Data
To ensure reproducibility, we provide all necessary input data and time-intensive intermediate results (e.g., outputs from gamlss model). All other results are generated by the shared codes and typically require minimal computation time, ranging from minutes to tens of minutes.  
...
All codes were tested on Windows 10 and CentOS Linux release 7.8. Please use the “add path” in MATLAB, "install.packages()" in R, and "pip install" in Python to add toolboxes and packages in the enviroment. All installations are very fast, typically completing within minutes.
